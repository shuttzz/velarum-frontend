---
name: release
description: Gera nova versão (SemVer automático), commita tudo, cria tag anotada e faz push para o remoto
---

Gere uma nova release do projeto atual. NUNCA pergunte o tipo de bump ao usuário — determine automaticamente analisando o histórico de commits.
...
- **NUNCA** adicionar `Co-Authored-By` ou qualquer assinatura do Claude em commits/tags.
...
- `git status`
- Leia o `package.json` (ou `Cargo.toml`, `pyproject.toml`, etc. — detecte o formato) e extraia a versão atual.
...
Analise os commits desde o último bump (`${LAST_VERSION_COMMIT}..HEAD`). Aplique a primeira regra que bater:

- **MAJOR** (x.0.0): qualquer commit com `BREAKING CHANGE:` no corpo OU `!` após tipo/escopo (ex: `feat!:`, `refactor(core)!:`).
- **MINOR** (0.x.0): pelo menos um `feat(...)` ou `feat:`.
- **PATCH** (0.0.x): apenas `fix`, `chore`, `docs`, `refactor`, `perf`, `style`, `test`, `build`, `ci` ou commits sem tipo convencional.

Se não houver commits desde o último bump, avise o usuário e pare.

### 3. Atualizar o arquivo de versão

Edite o campo de versão no manifest do projeto (`package.json`, `Cargo.toml`, `pyproject.toml`, etc.) para a nova versão calculada.

### 4. Stagear arquivos específicos

**NUNCA** use `git add -A` ou `git add .`. Adicione apenas os arquivos realmente modificados (identifique via `git status`). Avise antes de commitar se detectar arquivos sensíveis (`.env*`, `*.key`, `*credentials*`).

### 5. Commitar

Use o formato predominante no repositório. Default: `release: vX.Y.Z`.

```bash
git commit -m "release: vX.Y.Z"
```

Se pre-commit hook falhar, conserte e crie NOVO commit (nunca `--amend` em hook falho).

### 6. Criar tag anotada

```bash
git tag -a vX.Y.Z -m "vX.Y.Z - <resumo curto da principal mudança>"
```
...
Se a branch não tiver upstream, use `git push -u origin <branch>` no primeiro push.

### 8. Reportar

Responda em 2–4 linhas: versão anterior → nova versão, motivo do bump, confirmação de push dos commits e da tag.

## Confirmações antes de agir

Mostre ao usuário a versão calculada, o motivo e os arquivos a serem commitados. **Aguarde OK explícito** antes de `git commit`, `git tag` e `git push` — push é ação de alto impacto.
