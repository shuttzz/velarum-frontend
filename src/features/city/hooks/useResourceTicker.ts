...
export function useResourceTicker(city: City | undefined): Amounts | null {
  const snap = useRef<{ base: Amounts; rate: Amounts; at: number } | null>(null)
  const [value, setValue] = useState<Amounts | null>(null)

  useEffect(() => {
    if (city) snap.current = { base: city.resources, rate: city.rate, at: Date.now() }
  }, [city])

  useEffect(() => {
    const t = setInterval(() => {
      const s = snap.current
      if (s) setValue(extrapolate(s.base, s.rate, Date.now() - s.at))
    }, 250)
